    ...
    server.registerTool('invoke_pr_create', {
        ...
        try {
            ...
            if (!state?.work_branch || !state.work_branch_path) {
                return errorResponse(`Session ${session_id} has no work_branch - was it initialized via invoke_session_init_worktree?`);
            }
            ...
            try {
                const existing = execFileSync('gh', ['pr', 'view', workBranch, '--json', 'number,url'], { cwd, stdio: 'pipe' }).toString();
            ...
            try {
                ...
                return errorResponse(`gh pr create failed: ${err instanceof Error ? err.message : String(err)}`);
            }
        }
        catch (err) {
