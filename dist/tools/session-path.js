...
export function resolvePersistedSessionWorkBranchPath({ sessionId, projectDir, workBranch, workBranchPath, }) {
    ...
    if (!isSafeWorkBranch(workBranch, sessionId, workBranchPrefix)) {
        throw new Error(`Refusing to use session '${sessionId}': state.work_branch '${workBranch ?? '<unset>'}' does not match the expected session branch name`);
    }
    ...
    if (currentBranch !== workBranch) {
        throw new Error(`Refusing to use session '${sessionId}': worktree at '${canonicalPath}' is checked out on '${currentBranch}', expected '${workBranch}'`);
    }
