...
function sanitizedGitEnv(extra) {
    ...
    for (const [key, value] of Object.entries(process.env)) {
        if (SENSITIVE_GIT_ENV_VARS.includes(key))
            continue;
        env[key] = value;
    }
    if (extra) {
...
function validateBaseBranch(baseBranch) {
    if (!SAFE_REF_NAME.test(baseBranch)) {
        throw new Error(`Refusing to use base_branch '${baseBranch}': must match a conservative git ref name pattern`);
    }
...
function countCommitsSince(cwd, baseRef) {
    return parseInt(execFileSync('git', ['rev-list', '--count', `${baseRef}..HEAD`], {
        cwd,
        stdio: 'pipe',
        env: sanitizedGitEnv(),
    })
...
function isConflictFailure(rebaseError, conflictingFiles) {
    ...
    return (msg.includes('could not apply') ||
...
function requireSessionPath(sessionPath, sessionId) {
    ...
    return errorResponse(`Session ${sessionId} has no work_branch_path - was it initialized via invoke_session_init_worktree?`);
}
export function registerRebaseTools(server, sessionManager, projectDir) {
    server.registerTool('invoke_autosquash_session', {
    ...
    server.registerTool('invoke_collapse_commits', {
        ...
        try {
            const sessionPathOrError = requireSessionPath(await resolveSessionWorkBranchPath(sessionManager, projectDir, session_id), session_id);
    ...
    server.registerTool('invoke_get_commit_title', {
...
//# sourceMappingURL=rebase-tools.js.map