...
export async function checkForNewDefaults(projectDir) {
...
function describeDefault(relPath) {
    if (relPath.includes('roles/reviewer/'))
        return `New reviewer: ${path.basename(relPath, '.md')}`;
    if (relPath.includes('roles/researcher/'))
        return `New researcher: ${path.basename(relPath, '.md')}`;
    if (relPath.includes('roles/planner/'))
    ...
    if (relPath === 'context-template.md')
        return 'Project context template';
    return `New default: ${relPath}`;
}
//# sourceMappingURL=defaults-checker.js.map